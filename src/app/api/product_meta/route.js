...
export const GET = async () => {
  try {
    const postMetaData = await prisma.product_meta.findMany();
...
export const POST = async (req) => {
  try {
    ...
    const createMetaData = await prisma.product_meta.create({ data: body });
...
export const PUT = async (req) => {
  try {
    const body = await req.json();
    const { id } = body || {};
    const updatedData = await prisma.product_meta.update({
      where: { id: Number(id) },
      data: body,
    });
...
export const DELETE = async (req) => {
  try {
    const body = await req.json();
    const { id } = body;
    const deletedData = await prisma.product_meta.delete({
      where: { id: Number(id) },
    });
