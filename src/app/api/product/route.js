...
export const GET = async () => {
  try {
    const postMetaData = await prisma.products.findMany();
...
export const POST = async (req) => {
  try {
    ...
    const createMetaData = await prisma.products.create({ data: body });
...
export const DELETE = async (req) => {
  try {
    const body = await req.json();
    const { id } = body;
    const deletedData = await prisma.products.delete({
      where: { id: Number(id) },
    });
